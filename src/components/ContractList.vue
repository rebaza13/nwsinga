        ...
        <q-card v-for="contract in filteredContracts" :key="contract.id" flat bordered class="contract-card q-mb-md">
          <q-card-section>
            <div class="row items-center justify-between">
              <div class="text-subtitle1 text-weight-bold">{{ contract.propertyName }}</div>
              <q-badge :color="contract.isActive ? 'positive' : 'negative'">
                {{ contract.isActive ? 'Active' : 'Inactive' }}
              </q-badge>
            </div>
            <div class="text-caption">Tenant: {{ contract.tenantName }}</div>
            ...
            <div class="row q-mt-sm">
              <div class="col-6">
                ...
                <div class="text-body2 text-weight-medium text-primary">{{ formatCurrency(contract.monthlyRent) }}</div>
              </div>
              <div class="col-6">
      ...
      <q-card-section v-if="selectedContract" class="q-pt-lg">
        <div class="row q-col-gutter-md">
          ...
          <div class="col-12 q-mb-md">
            <div class="row items-center justify-between">
              <div class="text-h6">{{ selectedContract.propertyName }}</div>
              <q-badge :color="selectedContract.isActive ? 'positive' : 'negative'" class="q-pa-sm">
                {{ selectedContract.isActive ? 'Active' : 'Inactive' }}
              </q-badge>
            </div>
            <div class="text-subtitle2 text-grey-8">Tenant: {{ selectedContract.tenantName }}</div>
          </div>
          ...
          <div class="col-12 col-sm-6">
            <q-item dense>
              ...
              <q-item-section>
                ...
                <q-item-label class="text-weight-medium text-positive">{{ formatCurrency(selectedContract.monthlyRent)
                  }}</q-item-label>
              </q-item-section>
            </q-item>
          </div>
...
const columns = [
  { name: 'propertyName', label: 'Property', field: 'propertyName', sortable: true, align: 'left' },
  { name: 'tenantName', label: 'Tenant', field: 'tenantName', sortable: true, align: 'left' },
  {
    name: 'startDate',
    label: 'Start Date',
    field: 'startDate',
...
const filteredContracts = computed(() => {
  ...
  return contractStore.contracts.filter(contract =>
    ...
    contract.monthlyRent.toString().includes(searchLower)
  );
});
